...
- MCP-US-079 is `In Review / HIL Ready`: v0.19.0 implementeer 'n permanente `synthio` audio graph agter `SYNTHIO_BASELINE_ENABLED`. Die MIDI hot path roep net `synth.press()`/`synth.release()`; geen per-event `I2SOut.play()` of `I2SOut.stop()` nie. Hosttoetse: 152 groen; wag op menslike Logic/I2S hoorbaarheidstest.
